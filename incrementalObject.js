...
function Game(a_fps) {
	...
	this.getPointsPerClick = function () {
		return pointsPerClick;
	}; //return the main pointsPerClick that affects the score
...
function EntitySet(n_name, n_attributes) {
	...
	//public variables
	this.length = 0;
	this.entities_save = (function () {
		return entities;
	})();
	this.name_save = (function () {
		return name;
	})();
	//functions
	...
	}; //this needs to be deprecated with our next major version
...
AttributeSet.prototype.untrack = function (attr) {
	...
	if (typeof attr !== 'undefined') {
		...
		for (var i in Tracks)
			if (Tracks[i].container === this && Tracks[i].name == attr)
				delete Tracks[i];
